...
func SendString(s string) {
	...
	if state&0x0001 == 0 {
		...
		var caps_down INPUT
		caps_down.inputType = 1
		caps_down.ki.wVk = uint16(0x14)
		var caps_up INPUT
		caps_up.inputType = 1
		caps_up.ki.wVk = uint16(0x14)
		caps_up.ki.dwFlags = 0x0002

		inputs = append(inputs, caps_down)
		inputs = append(inputs, caps_up)
	}
	...
	for _, c := range s {
		...
		input_up.inputType = 1
		input_up.ki.wVk = uint16(vkc)
		input_up.ki.wScan = uint16(vsc)

		input_down.inputType = 1
		input_down.ki.wVk = uint16(vkc)
		input_down.ki.wScan = uint16(vsc)
		input_down.ki.dwFlags = 0x0002 //KEYEVENTF_KEYUP

		inputs = append(inputs, input_up)
		inputs = append(inputs, input_down)
	}
...
// SendString sends the specified string
func SendString2(s string) {
	// get keyboard layout for VkKeyScanExA
	hkl, _, _ := getKeyboardLayoutProc.Call(uintptr(0))
	// get caps lock state 0x14 is VK_CAPITAL
	state, _, _ := getKeyStateProc.Call(uintptr(0x14))
	...
	for _, c := range s {
		...
		vsc, _, _ := mapVirtualKeyExAProc.Call(vkCode, uintptr(0))
		fmt.Println(vkCode, vsc, shiftState)
		keydbProc.Call(vkCode, vsc, uintptr(0), 0)
		// _KEYEVENTF_KEYUP = 0x0002
		keydbProc.Call(vkCode, vsc, uintptr(0x002), 0)
	}
...
func downKey(key int) {
	...
	vkey := key + 0x80
	keydbProc.Call(uintptr(key), uintptr(vkey), uintptr(flag), 0)
}
func upKey(key int) {
